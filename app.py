...
import numpy as np
import pickle  # Use Python's built-in pickle (compatible with all versions)
import pandas as pd
...
# Load models with fallback - using built-in pickle
if missing_models:
    ...
    try:
        # Use Python's built-in pickle which works with Python 3.13
        ...
        st.error(f"Error loading models: {str(e)[:200]}...")
        models_loaded = False
        scaler = None
        kmeans_model = None
        dbscan_model = None
...
with st.sidebar:
    ...
    with st.expander("🔧 Debug Info"):
        ...
        st.write(f"Python pickle version: {pickle.format_version}")
        st.write(f"Missing files: {missing_models}")
...
# Try to use pre-trained DBSCAN model if available
if models_loaded and scaler is not None and dbscan_model is not None:
    try:
        user_scaled = scaler.transform(user_data)
        user_dbscan = dbscan_model.fit_predict(user_scaled)[0]
    except Exception as e:
        user_dbscan = -1
        st.sidebar.warning(f"DBSCAN prediction failed: {str(e)[:100]}")
else:
    user_dbscan = -1
    if not models_loaded:
        st.sidebar.info("Using demo mode - DBSCAN not available")
...
with col2:
    if user_dbscan == -1:
        cluster_display = "Outlier"
        cluster_description = "Demo mode / Anomaly"
    else:
        cluster_display = f"Cluster {user_dbscan}"
        cluster_description = "Normal pattern"
    
    st.markdown("""
    <div style='background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); 
                padding: 20px; border-radius: 10px; text-align: center;'>
        <h3 style='color: white; margin: 0;'>DBSCAN Result</h3>
...
with tab3:
    ...
    with col1:
        ...
        colors_bar = plt.cm.rainbow(np.linspace(0, 1, n_clusters))
...
if not models_loaded:
    ...
    **Demo Mode Active**
    ...
    The app is running in demo mode because model files weren't found or couldn't be loaded.
    To use the full functionality:
    
    1. Ensure these files are in your app directory:
       - `kmeans_model.pkl`
       - `dbscan_model.pkl` 
       - `scaler.pkl`
    
    2. Or run the app with demo data as shown above.
    """)
